            ...
            foreach($carts->render($customer_id) as $cart){
                ...
                if(isset($_POST['submit'])){
                    if(isset($_POST['payment'])){
                        ...
                        //Add items to order
                        if($count == 1){
                            $order->addOrder($customer_id, $totalItem[0], $_POST['grand_total']);
                        }

                        $order_item->addOrderItems($customer_id, $cart['food_id']);

                        //Updating cart
                        if($totalItem[0] == $count){
                            $carts->remove($customer_id, 'all');
