            ...
            <?php
                foreach($orders->custDetail('', $status['status'], 'vendor') as $order){
                    if($status['status'] == 'pending'){ //directory for pending orders
                        echo '<a href="order_pending.php?customer_id='.$order['user_id'].'&order_id='.$order['order_id'].'&status='.$order['status'].'" class="order-items '.$status['status'].'">';
                    }
                    else if($status['status'] == 'preparing'){ //directory for preparing orders
                        echo '<a href="order_preparing.php?customer_id='.$order['user_id'].'&order_id='.$order['order_id'].'&status='.$order['status'].'" class="order-items '.$status['status'].'">';
                    }
                    else if($status['status'] == 'ready for pickup'){ //directory for pickup orders
                        echo '<a href="order_ready.php?customer_id='.$order['user_id'].'&order_id='.$order['order_id'].'&status='.$order['status'].'" class="order-items '.$status['status'].'">';
                    }
            ?>
                            <div class="cont1">
