            ...
            <?php
                foreach($notifications->getAll($_SESSION['user_id']) as $notification){
                    if($notification['type'] === 'place order' || $notification['type'] === 'update order'){
                        $href = 'order_info.php?order_id=' . $notification['order_id'] . '&order_user_id=' . $order->getOrderInfo($notification['order_id'])['user_id'] . '&status=' . $order->getOrderInfo($notification['order_id'])['order_status'] . '&notif_id=' . $notification['notif_id'] . '&read=' . $notification['notif_read'];
