<?php
    ...
    class Cart{
        ...
        function addToCart($logged_in, $user_id, $food_id, $food_name, $food_price, $food_img){
            if($logged_in != 'null'){ //for logged in customer
                $sql = 'INSERT INTO cart (user_id, food_id, status) VALUES 
                    (:user_id, :food_id, "pending");';
        ...
        function render($user_id){
            $sql = 'SELECT cart.cart_id, cart.user_id, cart.food_id, cart.status, food_product.price, food_product.img, food_product.name FROM cart, food_product 
