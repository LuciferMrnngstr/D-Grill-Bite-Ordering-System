    ...
    class Order{
        ...
        function countCustOrder($user_id){
            if($user_id == 'all'){ //for vendors landing page
                $sql = 'SELECT COUNT(*) FROM orders WHERE order_status != "released";';
            }
        ...
        // this
        function renderOrdersVendor($order_status){
                $sql = 'SELECT orders.order_id, orders.user_id, orders.total_order, orders.grand_total, orders.order_status, orders.payment_method, orders.payment_status, orders.order_status, DATE_FORMAT(orders.created_at, "%M %d, %Y") as date, DATE_FORMAT(orders.created_at, "%H:%i") as time, customer.last_name, customer.first_name, customer.middle_name, customer.email, customer.contact_no 
                FROM orders, customer, user WHERE orders.user_id = user.user_id AND user.customer_id = customer.customer_id AND orders.order_status = :order_status ORDER BY orders.created_at;';
